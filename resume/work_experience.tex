...
\newcommand{\workSPK}{
  \cventry
  {
    \EN{Data and security governance, project management of security initiatives, IT risk and compliance assessments}
    \IT{Governo dei dati e della sicurezza, project management di iniziative di sicurezza, valutazioni di rischio IT e compliance normativa}
  }
  {
    \EN{Senior IT Security Officer -- Sparkasse}
    \IT{Unità Security -- Cassa di Risparmio di Bolzano}
  }
  {Bolzano (IT)} % Location
  {2021--2023} % Date(s)
  {
    \begin{cvitems}
      \item{
        \EN{Wrote and implemented security policies, performed IT risk assessments, reported to management/auditing functions}
        \IT{Scritto ed implementato policy di sicurezza, svolto valutazioni di rischio IT, presentato reportistica verso management e funzioni di controllo}
      }
      \item{
        \EN{Deployed and managed solutions for Endpoint Detection and Response (EDR), Data Loss Prevention, Breach & Attack Simulation, certificate management}
        \IT{Messo in linea e amministrato soluzioni Endpoint Detection and Response (EDR), Data Loss Prevention (DLP), Breach & Attack Simulation (BAS), gestione certificati}
      }
      \item{
        \EN{Managed and executed red and purple team activities; tracked and verified the remediation efforts}
        \IT{Coordinato ed eseguito attività di red e purple team; seguito e verificato le attività di rimedio}
      }
      \item{
        \EN{Detected and responded to incidents and frauds, providing assistance to law enforcement}
        \IT{Individuato e contrastato frodi informatiche, fornendo assistenza alle forze dell'ordine}
      }
      \item{
        \EN{Mentored interns and new hires, promoted a culture of communication between offices}
        \IT{Mentore per stagisti e nuovi assunti, promuovendo la comunicazione tra gli uffici}
      }
      \item{
        \EN{Wrote and implemented policies and procedures for a stronger data governance/data quality organizational framework}
        \IT{Scritto ed implementato policy e procedure per irrobustire il framework aziendale di data governance e data quality}
      }
    \end{cvitems}
  }
}
